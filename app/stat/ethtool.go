...
// ethtool.go is related to ethtool - link control and status
...
// EthtoolCmd repeats ethtool_cmd struct - link control and status - DEPRECATED struct
type EthtoolCmd struct { /* ethtool.c: struct ethtool_cmd */
	...
	PhyAddress    uint8  // MDIO address of PHY (transceiver)
	...
	MdioSupport   uint8  // Bitmask of %ETH_MDIO_SUPPORTS_* flags for the MDIO protocols
	...
	SpeedHi       uint16 // High bits of the speed, 1Mb units, 0 to INT_MAX or SPEED_UNKNOWN
	EthTpMdix     uint8  // Ethernet twisted-pair MDI(-X) status
	EthTpMdixCtrl uint8  // Ethernet twisted pair MDI(-X) control
	LpAdvertising uint32 // Bitmask of %ADVERTISED_* flags for the link modes and features
	Reserved      [2]uint32
}

// EthtoolLinkSettings repeats ethtool_link_settings struct - link control and status - NEWER struct
type EthtoolLinkSettings struct {
	...
	PhyAddress          uint8  // MDIO address of PHY (transceiver)
	Autoneg             uint8  // Enable/disable autonegotiation and auto-detection
	MdioSupport         uint8  // Bitmask of %ETH_MDIO_SUPPORTS_* flags for the MDIO protocols supported by the interface
	EthTpMdix           uint8  // Ethernet twisted-pair MDI(-X) status
	EthTpMdixCtrl       uint8  // Ethernet twisted pair MDI(-X) control
	LinkModeMasksNwords uint8  // Number of 32-bit words for each of the supported, advertising, lp_advertising link mode bitmaps.
