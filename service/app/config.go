...
type Config struct {
	...
	ServicesConnSettings []model.ServiceConnSetting `json:"services"` // Slice of connection settings for exact services
...
func (c *Config) Validate() error {
	...
	if c.ServicesConnSettings != nil {
		...
		if foundInvalid {
			cc := make([]model.ServiceConnSetting, 0, len(c.ServicesConnSettings))
