...
const (
	pgVersionNumQuery = "SELECT current_setting('server_version_num')::int"
...
FROM pg_show_all_settings()`
	...
	pgSchemaFkeyColumnsMismatch    = `SELECT current_database() AS datname, c1.relnamespace::regnamespace::text AS schemaname, c1.relname AS relname, a1.attname||'::'||t1.typname AS colname, c2.relnamespace::regnamespace::text AS refschemaname, c2.relname AS refrelname, a2.attname||'::'||t2.typname AS refcolname, 1 AS exists FROM pg_constraint JOIN pg_class c1 ON c1.oid=conrelid JOIN pg_class c2 ON c2.oid=confrelid JOIN pg_attribute a1 ON a1.attnum=conkey[1] AND a1.attrelid=conrelid JOIN pg_attribute a2 ON a2.attnum=confkey[1] AND a2.attrelid=confrelid JOIN pg_type t1 ON t1.oid=a1.atttypid JOIN pg_type t2 ON t2.oid=a2.atttypid WHERE a1.atttypid<>a2.atttypid AND contype='f'`
)
