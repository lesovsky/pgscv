...
func TestServiceRepo_addService(t *testing.T) {
	r := NewServiceRepo()
	s1 := model.TestSystemService()
	s2 := model.TestPostgresService()
	s3 := model.TestPgbouncerService()
...
func TestServiceRepo_addServicesFromConfig(t *testing.T) {
	testCases := []struct {
		...
		{
			name: "valid",
			config: &Config{ServicesConnSettings: []model.ServiceConnSetting{
		...
		},
		{
			...
			config:   &Config{ServicesConnSettings: []model.ServiceConnSetting{{ServiceType: "postgres", Conninfo: "invalid conninfo"}}},
			expected: 1,
		},
		{
			name:     "unavailable service",
			config:   &Config{ServicesConnSettings: []model.ServiceConnSetting{{ServiceType: "postgres", Conninfo: "port=15432"}}},
			expected: 1,
		},
	}
